...
const schema = z.object({
  ...
  safetyAccepted: z.boolean().refine((value) => value, 'Please accept the safety responsibility notice'),
});
...
function getErrorMessage(error: unknown) {
  if (typeof error === 'object' && error !== null) {
    ...
    return candidate.response?.data?.message || candidate.message || 'Unable to submit listing';
  }

  return 'Unable to submit listing';
}
...
export default function PostServicePage() {
  ...
  const stepReady: Record<number, boolean> = useMemo(
    () => ({
      1: Boolean(values.title && values.description && values.city && files.length > 0),
      2: Boolean(values.whatsapp),
      3: Boolean(values.safetyAccepted),
    }),
    [values, files]
  );
  ...
  function handleFilesChange(event: ChangeEvent<HTMLInputElement>) {
    ...
    const nextFiles = [...files, ...selected].slice(0, MAX_PHOTOS);
    if (files.length + selected.length > MAX_PHOTOS) {
      toast.error(`You can upload a maximum of ${MAX_PHOTOS} images.`);
    }
            ...
            <div className="grid gap-4 md:grid-cols-2">
              ...
              <div className="md:col-span-2 space-y-3">
                <div>
                  ...
                  <input type="file" accept="image/*" multiple className="input" onChange={handleFilesChange} />
                  <p className="mt-2 text-xs text-slate-500">Upload 1 to 4 photos. Files are capped at {MAX_PHOTOS} images and will be optimized automatically for faster loading.</p>
                </div>
                    ...
                    {previews.map((preview, index) => (
                      <div key={`${preview.file.name}-${index}`} className="overflow-hidden rounded-2xl border border-slate-200 bg-white">
                        ...
                        <div className="space-y-2 p-3">
                          ...
                          <button type="button" className="w-full rounded-xl border border-rose-200 px-3 py-2 text-xs font-semibold text-rose-600" onClick={() => removeFile(index)}>
                            Remove image
                          </button>
                        </div>
                      </div>
